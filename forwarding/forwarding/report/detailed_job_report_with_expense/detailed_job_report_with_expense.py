...
def get_result(filters, account_details):
	...
	list(map(update_direct_payment_id,gl_entries))
...
def update_direct_payment_id(gl_entry):
	jv = gl_entry['voucher_no']
	direct_payment_id = frappe.db.get_value("Journal Entry",jv,"direct_payment_id")
	if gl_entry['voucher_type'] in ["Journal Entry", "Cash Entry", "Bank Entry"] and direct_payment_id:
		gl_entry['voucher_type'] = "Direct Payment"
		gl_entry['voucher_no'] = direct_payment_id

def get_conditions(filters):
	conditions = []
	expense_accounts = set(frappe.db.get_all("Account",filters={"account_type":"Expense Account"},pluck='name'))
	conditions.append("""account in (select name from tabAccount
		where account_type in ('Income Account','Expense Account','Cost of Goods Sold'))""")
...
def get_result_as_list(data, filters):
	...
	inv_detail = get_customer_invoice_details()
	...
	for d in data:
		...
		if d.get('party_type')=="Customer":
			d['bill_no'] = inv_detail.get(d.get('voucher_no'), '')
		elif d.get('party_type')=="Supplier":
			d['bill_no'] = inv_details.get(d.get('voucher_no'), '')

	return data

def get_customer_invoice_details():
	inv_detail = {}
	for d in frappe.db.sql(""" select name, po_no from `tabSales Invoice`
		where docstatus = 1 and po_no is not null and po_no != '' """, as_dict=1):
		inv_detail[d.name] = d.po_no
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Income ({0})").format(currency),
			"fieldname": "credit",
			"fieldtype": "Float",
			"width": 100
		},
		{
			"label": _("Expense ({0})").format(currency),
			"fieldname": "debit",
			"fieldtype": "Float",
			"width": 100
		},
		{
			"label": _("Profit or Loss ({0})").format(currency),
			"fieldname": "balance",
			"fieldtype": "Float",
			"width": 130
		}
