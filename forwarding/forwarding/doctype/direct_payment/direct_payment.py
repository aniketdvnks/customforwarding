...
class DirectPayment(Document):
	def on_submit(self):
		...
		pr = frappe.get_doc("Direct Payment",self.name)
		for i in pr.shipping_payment:
			update_data(i.shipment_payment_request,i,pr)
	...
	def on_cancel(self):
		frappe.db.sql("""update `tabShipment Payment Request`
		set payment_id=null, status='Pending'
		where name=%s """,self.name)
		frappe.db.commit()
	
			
		jvs = frappe.db.get_all("Journal Esntry",filters={"direct_payment_id":self.name})
		if jvs:
			for row in jvs:
				jv = frappe.get_doc("Journal Entry",row.name)
				jv.cancel()
				frappe.db.commit()
				frappe.msgprint("Canceling Linked JV")	


def update_data(dp,i,pr):
	c = frappe.get_doc("Shipment Payment Request",dp)
	c.status = 'Paid'
	c.payment_id = pr.name
	c.save()
	frappe.db.commit()
	return frappe.msgprint(_("{0} updated").format(c))		

def update_cancel(dp,i,pr):
	c = frappe.get_doc("Shipment Payment Request",dp)
	c.status = 'Pending'
	c.payment_id = ''
	c.save()
...
@frappe.whitelist()
def make(docname, method=None):
	pr = frappe.get_doc("Direct Payment",docname)
	for i in pr.shipping_payment:
		update_data(i.shipment_payment_request,i,pr)
